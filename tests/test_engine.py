...
def test_rest_recovery_converges_instead_of_ratchets():
    now = [0.0]
    engine = EmotionStateEngine(clock=lambda: now[0])
    engine.baselines.update({"dopamine": 50, "serotonin": 60, "cortisol": 20})
    now[0] = 12 * 3600
    engine.apply_decay(hour=12)
    first = dict(engine.baselines)
    for _ in range(1, 20):
        now[0] += 12 * 3600
        engine.apply_decay(hour=12)
    assert engine.baselines["dopamine"] == pytest.approx(58)
    assert engine.baselines["serotonin"] == pytest.approx(66)
    assert engine.baselines["cortisol"] == pytest.approx(15)
    assert engine.baselines == first


def test_dominant_signals_use_effective_baseline():
    engine = EmotionStateEngine()
    engine.state["serotonin"] = 68
    assert engine.dominant_signals(hour=18)["serotonin"] == pytest.approx(0)


def test_voice_reference_resists_habituation_to_persistent_loudness():
    engine = EmotionStateEngine()
    engine.on_voice_tone(1.0, 0)
    initial = engine._voice_norm
    hard_events = 0
    for _ in range(100):
        before = len(engine.events)
        engine.on_voice_tone(2.0, 1.0)
        hard_events += any(event.name == "voice_tone_hard" for event in engine.events[before:])
    assert hard_events >= 90
    assert engine._voice_norm < 1.25
    assert engine._voice_norm > initial


def test_voice_reference_can_recover_downward():
    engine = EmotionStateEngine()
    engine.on_voice_tone(2.0, 0)
    start = engine._voice_norm
    for _ in range(100):
        engine.on_voice_tone(0.5, 0)
    assert engine._voice_norm < start


def test_corrupt_state_raises_clear_error(tmp_path):
    path = tmp_path / "state.json"
    path.write_text("{broken")
    with pytest.raises(ValueError, match="Could not load emotion state"):
        EmotionStateEngine(path)
